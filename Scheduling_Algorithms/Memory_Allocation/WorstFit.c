...
int allocate_memory(int size) {
  int i, max_size = -1, max_index = -1;
  for (i = 0; i < num_partitions; i++) {
    if (partitions[i] >= size && partitions[i] > max_size) {
  ...
  if (max_index == -1) {
    ...
    partitions[max_index] -= size; // reduce partition size
    return max_index; // return partition index
  }
}
